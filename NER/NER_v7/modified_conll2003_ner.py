...
class NERAnnotator(CombinedAnnotator):
    ...
    def add_shallow(self):
        ...
        self.add_annotator(FunctionAnnotator("custom_lf", custom_lf_func))
...
def custom_lf_func(doc):
    ...
    for span in sorted(spans):
        ...
        if span_text in labelling_functions_data.cased_misc:
            yield span.start, span.end, "MISC"

        elif span_text in labelling_functions_data.cased_org:
            yield span.start, span.end, "ORG"

        span_text = span_text.lower()

        if span_text in labelling_functions_data.not_ents:
            yield span.start, span.end, "NOT_ENT"

        elif span_text in labelling_functions_data.sg_loc:
            yield span.start, span.end, "LOC"

        elif span_text in labelling_functions_data.all_misc:
            yield span.start, span.end, "MISC"

        elif span_text in labelling_functions_data.all_org:
            yield span.start, span.end, "ORG"
