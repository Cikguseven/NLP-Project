...
def custom_lf_func(doc):
    ...
    for span in sorted(spans):
        ...
        if span_text in LF_DATABASE.cased_misc:
            yield span.start, span.end, "MISC"

        elif span_text in LF_DATABASE.cased_org:
            yield span.start, span.end, "ORG"

        span_text = span_text.lower()

        if span_text in LF_DATABASE.not_ents:
            yield span.start, span.end, "NOT_ENT"

        elif span_text in LF_DATABASE.sg_loc:
            yield span.start, span.end, "LOC"

        elif span_text in LF_DATABASE.all_misc:
            yield span.start, span.end, "MISC"

        elif span_text in LF_DATABASE.all_org:
            yield span.start, span.end, "ORG"
